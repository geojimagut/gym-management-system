...
Class Action {
	...
	function login(){
			...
			extract($_POST);		
			$qry = $this->db->query("SELECT * FROM user_client inner join members on user_client.member_id=members.id and username = '".$username."' and password = '".$password."' ");
			if($qry->num_rows > 0){
	...
	function login2(){
			...
			if(isset($email))
				$username = $email;
		$qry = $this->db->query("SELECT * FROM user_client where username = '".$username."' and password = '".$password."' ");
		if($qry->num_rows > 0){
				// $theresult=mysqli_fetch_assoc($qry);
				// $_SESSION['user_id']=$theresult['id'];
	...
	function get_schecdule(){
		...
		extract($_POST);
		$data = array();
		$qry = $this->db->query("SELECT s.*,concat(m.lastname,',',m.firstname,' ', m.middlename) as name FROM schedules s inner join members m on m.id = s.member_id and m.id='$memberid' ");
