		...
		<div class="col-md-9">
			<table class="table table-bordered">
				...
				<tbody>
					<?php 
						$pcount=0;
					$paid = $conn->query("SELECT * FROM registration_info inner join plans on registration_info.plan_id = plans.id inner join members on members.id=registration_info.member_id inner join trainers on registration_info.trainer_id=trainers.id");
					...
					<tr>
						...
						<td><?php if($row['status']==1){
							...
							?><i class="fa fa-file confirm"style="color:rgb(0,130,189);cursor:pointer;font-size:18px;"></i> <span><i class="fa fa-trash "style="color:red;cursor:pointer;font-size:18px;margin-left:10px"></span><?php
						}else{
							?><span style="font-size:14px;color:green;margin-right:5px;">PAID</span><?php
							?><i class="fa fa-trash"style="color:red;cursor:pointer;font-size:18px;"></i><?php
						}
						 ?></td>
					</tr>
					<?php endwhile; ?>
...
<script>
	...
	$('.confirm').on('click', function(){
		...
		var state=confirm('Confirm this payment?')
	...
	$('.fa-trash').on('click', function(){
		var id=$(this).closest('tr').find('td:eq(0)').text().trim()
		var state=confirm('Delete this record?')
